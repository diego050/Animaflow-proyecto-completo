...
export const StyleChip: React.FC<StyleChipProps> = ({
  ...
  const customBoxShadow = style?.boxShadow ? `${(style.boxShadow as any).x || 0}px ${(style.boxShadow as any).y || 2}px ${(style.boxShadow as any).blur || 8}px ${(style.boxShadow as any).spread || 0}px ${(style.boxShadow as any).color || 'rgba(0,0,0,0.2)'}` : '0 2px 8px rgba(0,0,0,0.15)';
  ...
  return (
    ...
    >
      {icon && <IconifyIcon name={icon} size={s.iconSize} color={customColor} />}
      {text}
      {deletable && (
