...
export const FunnelChart: React.FC<FunnelChartProps> = ({
  ...
  // Relativo al lienzo (antes px: layerHeight 100, maxWidth 600, fontSize 18/32).
  const layerHeight = c.vmin(15);
  const maxWidth = c.vw(80);

  return (
    <div style={{ position: 'absolute', top: `${y}px`, left: `${x}px`, transform: `translate(-50%, -50%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: `${c.vmin(1.4)}px`, fontFamily: 'Inter, sans-serif', zIndex: 40 }}>
      {valArr.map((val, idx) => {
        ...
        return (
          <div key={idx} style={{ position: 'relative', width: `${width * anim}px`, height: `${layerHeight}px`, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: anim }}>
              <div style={{ fontSize: `${c.vmin(4.4)}px`, fontWeight: 900, color: textColor, textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>{val.toLocaleString()}</div>
              <div style={{ fontSize: `${c.vmin(2.8)}px`, fontWeight: 600, color: 'rgba(255,255,255,0.8)', textTransform: 'uppercase', letterSpacing: '1px' }}>{labelArr[idx]}</div>
            </div>
          </div>
        );
      })}
