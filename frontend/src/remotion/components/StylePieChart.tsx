...
export const StylePieChart: React.FC<StylePieChartProps> = ({
  ...
  return (
    <div style={{ position: 'absolute', top: `${y}px`, left: `${x}px`, transform: 'translate(-50%, -50%)', opacity: customOpacity, zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        {data.map((slice, i) => {
          ...
          const ir = variant === 'donut' ? innerRadius : 0;
          ...
          const explodeOffset = explodeSlice === i ? 8 : 0;
      ...
      {(showLabels || showValues) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 200 }}>
          {data.map((slice, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <div style={{ width: 8, height: 8, borderRadius: 2, backgroundColor: slice.color ?? defaultColors[i % defaultColors.length] }} />
