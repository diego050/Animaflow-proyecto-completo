...
export const StyleVideoPlayer: React.FC<StyleVideoPlayerProps> = ({
  ...
  if (!src) {
    return (
      <div style={{ position: 'absolute', top: `${y}px`, left: `${x}px`, transform: `translate(-50%, -50%) scale(${scale})`, opacity: customOpacity, zIndex: 50, width: customWidth, height: customHeight, backgroundColor: '#1E293B', borderRadius: `${customBorderRadius}px`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748B', fontFamily: 'Inter, sans-serif', fontSize: 14 }}>
        No video source
      </div>
    );
  }
